...
.stamp-firmwares: .stamp-compiled
	...
	README_FILE=$(FW_TARGET_DIR)/README-kathleen-$$GIT_BRANCH_ESC.txt; \
	echo "git branch \"$(GIT_BRANCH)\", revision $(REVISION)" > $$README_FILE; \
	echo "https://github.com/freifunk-berlin/firmware" >> $$README_FILE; \
	echo "https://wiki.freifunk.net/Berlin:Firmware" >> $$README_FILE; \
	...
	for DIR_ABS in $(IB_BUILD_DIR)/imgbldr/bin/*; do \
	  ...
	  cp $(FW_TARGET_DIR)/README-kathleen-* $$TARGET_DIR/; \
